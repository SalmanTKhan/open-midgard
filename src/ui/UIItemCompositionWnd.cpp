...
UIItemCompositionWnd::UIItemCompositionWnd()
    : m_cardItemIndex(0),
      m_itemIndices(),
      ...
      m_iconCache()
{
    ...
    int savedX = m_x;
    int savedY = m_y;
    if (LoadUiWindowPlacement("ItemCompositionWnd", &savedX, &savedY)) {
...
RECT UIItemCompositionWnd::GetRowRect(int visibleRow) const
{
...
RECT UIItemCompositionWnd::GetScrollTrackRect() const
{
    const RECT listRect = GetListRect();
    return shopui::MakeRect(listRect.right + kScrollBarGap, listRect.top, kScrollBarWidth, listRect.bottom - listRect.top);
}

RECT UIItemCompositionWnd::GetScrollThumbRect(int itemCount) const
{
...
RECT UIItemCompositionWnd::GetOkButtonRect() const
{
...
RECT UIItemCompositionWnd::GetCancelButtonRect() const
{
    const int y = m_y + m_h - kPadding - kButtonHeight;
...
int UIItemCompositionWnd::GetMaxViewOffset(int itemCount) const
{
    return (std::max)(0, itemCount - kVisibleRows);
}

bool UIItemCompositionWnd::IsScrollBarVisible(int itemCount) const
{
    return itemCount > kVisibleRows;
}

void UIItemCompositionWnd::ClampViewOffset(int itemCount)
{
    m_viewOffset = std::clamp(m_viewOffset, 0, GetMaxViewOffset(itemCount));
}

void UIItemCompositionWnd::UpdateScrollFromThumbPosition(int globalY, int itemCount)
{
    const RECT trackRect = GetScrollTrackRect();
...
std::vector<const ITEM_INFO*> UIItemCompositionWnd::GetAvailableItems() const
{
    ...
    for (unsigned int itemIndex : m_itemIndices) {
        ...
        if (!item || item->m_itemIndex == 0 || item->m_num <= 0) {
            continue;
        }
...
const shopui::BitmapPixels* UIItemCompositionWnd::GetItemIcon(const ITEM_INFO& item)
{
...
bool UIItemCompositionWnd::HasValidSelection() const
{
    if (m_cardItemIndex == 0 || m_selectedItemIndex == 0) {
        return false;
    }

    const ITEM_INFO* cardItem = g_session.GetInventoryItemByIndex(m_cardItemIndex);
    const ITEM_INFO* equipItem = g_session.GetInventoryItemByIndex(static_cast<unsigned int>(m_selectedItemIndex));
    return cardItem && equipItem && cardItem->m_itemIndex != 0 && equipItem->m_itemIndex != 0;
}

int UIItemCompositionWnd::HitTestVisibleRow(int x, int y) const
{
...
void UIItemCompositionWnd::UpdateInteraction(int x, int y)
{
...
void UIItemCompositionWnd::SetCardItemIndex(unsigned int itemIndex)
{
    m_cardItemIndex = itemIndex;
    Invalidate();
}

void UIItemCompositionWnd::SetEquipmentItemIndices(const std::vector<unsigned int>& itemIndices)
{
...
std::string UIItemCompositionWnd::BuildTitle() const
{
    std::string title = "Compound Item";
    if (const ITEM_INFO* cardItem = g_session.GetInventoryItemByIndex(m_cardItemIndex)) {
        if (cardItem->m_itemIndex != 0) {
            title += " (";
            title += cardItem->GetDisplayName();
            title += ")";
        }
    }
    return title;
...
void UIItemCompositionWnd::SubmitSelection()
{
    if (!HasValidSelection()) {
        return;
    }

    g_modeMgr.SendMsg(CGameMode::GameMsg_RequestComposeCardIntoEquipment,
        static_cast<msgparam_t>(m_cardItemIndex),
        static_cast<msgparam_t>(m_selectedItemIndex),
        0);
    g_windowMgr.DeleteWindow(this);
}

void UIItemCompositionWnd::CancelSelection()
{
    g_modeMgr.SendMsg(CGameMode::GameMsg_RequestComposeCardIntoEquipment,
        static_cast<msgparam_t>(-1),
        static_cast<msgparam_t>(-1),
        0);
    g_windowMgr.DeleteWindow(this);
}

void UIItemCompositionWnd::OnDraw()
{
    if (m_show == 0) {
        return;
    }
    ...
    if (itemCount <= 0) {
        shopui::DrawWindowTextRect(hdc,
                                   listRect,
                                   "No equipment can accept this card.",
                                   RGB(80, 80, 80),
        ...
        for (int drawIndex = 0; drawIndex < visibleCount; ++drawIndex) {
            ...
            shopui::DrawWindowTextRect(hdc,
                                       nameRect,
                                       shopui::GetItemDisplayName(*item).c_str(),
                                       RGB(0, 0, 0),
            ...
            if (item->m_num > 1) {
                ...
                shopui::DrawWindowTextRect(hdc,
                                           countRect,
                                           std::to_string(item->m_num).c_str(),
                                           RGB(90, 64, 32),
...
void UIItemCompositionWnd::OnLBtnDblClk(int x, int y)
{
    const int hitIndex = HitTestVisibleRow(x, y);
    if (hitIndex < 0) {
        OnLBtnDown(x, y);
...
void UIItemCompositionWnd::OnLBtnDown(int x, int y)
{
...
void UIItemCompositionWnd::OnLBtnUp(int x, int y)
{
...
void UIItemCompositionWnd::OnMouseMove(int x, int y)
{
...
void UIItemCompositionWnd::OnMouseHover(int x, int y)
{
    UpdateInteraction(x, y);
    Invalidate();
}

void UIItemCompositionWnd::OnWheel(int delta)
{
    const int itemCount = static_cast<int>(GetAvailableItems().size());
    if (delta > 0) {
        --m_viewOffset;
...
void UIItemCompositionWnd::StoreInfo()
{
    SaveUiWindowPlacement("ItemCompositionWnd", m_x, m_y);
}

bool UIItemCompositionWnd::GetDisplayDataForQt(DisplayData* outData) const
{
    if (!outData) {
        return false;
    }
    ...
    data.emptyText = "No equipment can accept this card.";
    ...
    for (int drawIndex = 0; drawIndex < visibleCount; ++drawIndex) {
        ...
        row.label = shopui::GetItemDisplayName(*item);
