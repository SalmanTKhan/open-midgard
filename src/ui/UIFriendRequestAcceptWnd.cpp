...
UIFriendRequestAcceptWnd::UIFriendRequestAcceptWnd()
    : m_inviterAid(0),
      m_inviterCid(0),
      m_hoverButtonId(0),
      m_pressedButtonId(0),
...
UIFriendRequestAcceptWnd::~UIFriendRequestAcceptWnd() = default;

void UIFriendRequestAcceptWnd::SetShow(int show)
{
...
bool UIFriendRequestAcceptWnd::IsUpdateNeed()
{
...
void UIFriendRequestAcceptWnd::OnCreate(int x, int y)
{
    (void)x;
    (void)y;
    ...
    Create(kWindowWidth, kWindowHeight);
    m_id = UIWindowMgr::WID_FRIENDREQUESTACCEPTWND;
...
void UIFriendRequestAcceptWnd::OnDraw()
{
    ...
    FillRectColor(hdc, titleBar, RGB(92, 96, 111));
    FrameRectColor(hdc, titleBar, RGB(60, 62, 74));
...
void UIFriendRequestAcceptWnd::OnLBtnDown(int x, int y)
{
    m_pressedButtonId = HitTestButton(x, y);
    Invalidate();
}

void UIFriendRequestAcceptWnd::OnLBtnUp(int x, int y)
{
    ...
    if (pressedButtonId != 0 && pressedButtonId == releasedButtonId) {
        if (releasedButtonId == ButtonClose) {
            // Close button on the title bar is treated as "decline so the inviter
            // is not left waiting" — same convention the party invite window uses.
            SubmitReply(false);
        } else {
...
void UIFriendRequestAcceptWnd::OnMouseMove(int x, int y)
{
...
void UIFriendRequestAcceptWnd::OnKeyDown(int virtualKey)
{
    switch (virtualKey) {
    case VK_RETURN:
        SubmitReply(true);
...
bool UIFriendRequestAcceptWnd::HandleKeyDown(int virtualKey)
{
...
bool UIFriendRequestAcceptWnd::GetDisplayDataForQt(DisplayData* outData) const
{
    if (!outData) {
        return false;
    }
    ...
    outData->title = "Friend Request";
    outData->bodyText = nameLine + "\nWants to add you as a friend";
    outData->messageLines.clear();
    outData->messageLines.push_back(nameLine);
    outData->messageLines.push_back("Wants to add you as a friend");
...
RECT UIFriendRequestAcceptWnd::GetCloseButtonRect() const
{
    return RECT{ m_x + m_w - 18, m_y + 3, m_x + m_w - 18 + kCloseButtonWidth, m_y + 3 + kCloseButtonHeight };
}

RECT UIFriendRequestAcceptWnd::GetButtonRect(int buttonId) const
{
...
int UIFriendRequestAcceptWnd::HitTestButton(int x, int y) const
{
...
void UIFriendRequestAcceptWnd::ClearState()
{
    m_inviterAid = 0;
    m_inviterCid = 0;
    m_inviterName.clear();
...
bool UIFriendRequestAcceptWnd::SubmitReply(bool accept)
{
...
unsigned long long UIFriendRequestAcceptWnd::BuildVisualStateToken() const
{
    ...
    mix(static_cast<unsigned long long>(m_inviterAid));
    mix(static_cast<unsigned long long>(m_inviterCid));
