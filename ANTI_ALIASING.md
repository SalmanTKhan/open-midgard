...
- D3D11 now allocates and runs the SMAA edge-detection pass against the offscreen 3D scene target when `SMAA` is selected internally; the later blend-weight and neighborhood passes are still pending before the mode is exposed.
- The remaining SMAA work is focused on the later pass sequencing and final composition correctness.
